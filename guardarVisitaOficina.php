...
$lineasExtra = [
  ...
  "GPS Estado: {$estadoGPS}  Acc: {$accuracy}m  Sync: {$tiempoFormateado}",
];
...
$sql = "INSERT INTO GEOCAMPO_VISITA_OFICINA
(
  idpersonal,
  funcionario_nombre,
...
if (!$stmt) {
  error_log("[guardarVisitaOficina] prepare error: " . $mysqli->error);
...
if (!$stmt->execute()) {
  error_log("[guardarVisitaOficina] execute error: " . $stmt->error);
...
echo json_encode([
  'success' => true,
  'message' => 'Registro Visita Oficina ingresado correctamente.',
  'id' => $idInsert,
  'ruta_foto' => $ruta_foto,
