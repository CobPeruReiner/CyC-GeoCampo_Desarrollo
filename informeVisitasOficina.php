...
if (!in_array($cargoPersonal, $cargosAdmin)) {
  header("Location: formRegistroVisitaOficina.php");
  exit;
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Informe | Visitas Oficina</title>
        ...
        <table class="w-full text-sm text-gray-600">
          <thead class="text-xs uppercase text-gray-400 border-b">
            <tr>
              ...
              <th class="py-3 px-4 text-left">Teléfono Funcionario</th>
              ...
              <th class="py-3 px-4 text-center">Estado GPS</th>
    ...
    <div id="modalVisita"
      ...
      <div
        ...
        <button onclick="cerrarModal()"
          class="absolute top-3 right-3 text-gray-500 hover:text-gray-700 cursor-pointer text-lg">
          ✕
        </button>

        <iframe src="formRegistroVisitaOficina.php?modal=1"
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      async function fetchDataSimulada(filtros = {}) {
        ...
        try {
          ...
          const resp = await fetch("api/informeVisitasOficina.php?" + params.toString());
      ...
      function renderTable() {
        ...
        pageData.forEach(row => {
          ...
          if (row.estado_gps === "EXCELENTE") bgEstado = "bg-green-100 text-green-700";
          else if (row.estado_gps === "ACEPTABLE") bgEstado = "bg-yellow-100 text-yellow-700";
          else if (row.estado_gps === "MALO") bgEstado = "bg-red-100 text-red-700";
          else bgEstado = "bg-gray-100 text-gray-600";

          tablaBody.innerHTML += `
            <tr class="border-b hover:bg-gray-50 text-sm">
              <td class="py-3 px-4">${row.fecha}</td>
              <td class="py-3 px-4">${row.funcionario}</td>
              <td class="py-3 px-4">${row.funcionario_telefono}</td>
              <td class="py-3 px-4">${row.gestor}</td>
              <td class="py-3 px-4">${row.unidad}</td>
              <td class="py-3 px-4">${row.agencia}</td>
              <td class="py-3 px-4 text-center">
                <span class="px-2 py-1 rounded text-xs font-semibold ${bgEstado}">
                  ${row.estado_gps}
                </span>
              </td>
              <td class="py-3 px-4 text-center">
      ...
      window.exportar = function() {
        ...
        const nombreArchivo = `Informe_Visitas_Oficina_${fecha}_${hora}.xlsx`;
