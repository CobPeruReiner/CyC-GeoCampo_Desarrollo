...
$sql = "
  SELECT 
      v.id_GEOCAMPO_OPERATIVO,

      DATE_FORMAT(v.fecha_creacion_dispositivo,'%Y-%m-%d %H:%i:%s') AS fecha,

      v.documento_cliente AS dni,

      COALESCE(c.NOMBRE,'APOYO') AS cliente,
  ...
  FROM GEOCAMPO_OPERATIVO v

  LEFT JOIN C_FINANCIERA_CONFIANZA_CAMPO c
...
if ($fInicio) {
  $sql .= " AND DATE(v.fecha_creacion_dispositivo) >= ?";
  $params[] = $fInicio;
  $types .= "s";
}
...
$sql .= " ORDER BY v.fecha_creacion_dispositivo DESC";
